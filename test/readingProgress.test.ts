...
describe('reading progress', () => {
  it('maps the complete chapter archive', () => {
    expect(episodeCountForSeason(1)).toBe(10);
    expect(episodeCountForSeason(63)).toBe(10);
    expect(episodeCountForSeason(64)).toBe(3);
    expect(episodeCountForSeason(74)).toBe(10);
    expect(episodeCountForSeason(TOTAL_SEASONS)).toBe(10);
    expect(allReadingTargets()).toHaveLength(TOTAL_CHAPTERS);
  });
  ...
  it('calculates season and overall progress from opened episode ids', () => {
    ...
    expect(overallReadingProgress(read)).toMatchObject({ read: 3, total: TOTAL_CHAPTERS });
  });
  ...
  it('wraps to an earlier unread gap after later episodes have been opened', () => {
    ...
    expect(nextUnreadTarget(read, { season: TOTAL_SEASONS, episode: 10 })).toMatchObject({ season: 1, episode: 2 });
  });
});
