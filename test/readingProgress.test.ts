...
describe('reading progress', () => {
  ...
  it('calculates season and overall progress from opened episode ids', () => {
    ...
    expect(overallReadingProgress(read)).toMatchObject({ read: 3, total: 633 });
  });
  ...
  it('wraps to an earlier unread gap after later episodes have been opened', () => {
    ...
    expect(nextUnreadTarget(read, { season: 64, episode: 3 })).toMatchObject({ season: 1, episode: 2 });
  });
});
