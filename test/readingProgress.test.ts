...
describe('reading progress', () => {
  it('maps the complete 733-chapter archive', () => {
    ...
    expect(allReadingTargets()).toHaveLength(733);
  });
  ...
  it('calculates season and overall progress from opened episode ids', () => {
    ...
    expect(overallReadingProgress(read)).toMatchObject({ read: 3, total: 733 });
  });
  ...
  it('wraps to an earlier unread gap after later episodes have been opened', () => {
    ...
    expect(nextUnreadTarget(read, { season: 74, episode: 10 })).toMatchObject({ season: 1, episode: 2 });
  });
});
