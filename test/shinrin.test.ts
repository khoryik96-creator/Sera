...
describe('Shinrin roster', () => {
  it('locks exactly the ten official Paragons in canonical order', () => {
    ...
    expect(shinrinParagons.map((f) => f.name)).toEqual([
      'Tsubasa Kurokawa', 'Jin Takamori', 'Haru Ishikawa', 'Hana Arakawa', 'Kai Moriyama',
      'Aya Katsuragi', 'Kenji Narukami', 'Jun Kajihara', 'Eirik Voss', 'Nao Shibasaki',
    ]);
  });

  it('gives each Paragon a locked tier and a named Paragon Domain', () => {
    for (const figure of shinrinParagons) {
      expect(SHINRIN_TIER_ORDER, `${figure.key} tier`).toContain(figure.strength);
      expect(figure.affiliation).toBe('Shinrin');
      ...
      // Three signature skills plus the Domain.
      expect(figure.skills?.length, `${figure.key} skills`).toBe(4);
    }
  });
  ...
  it('matches the locked tier assignments', () => {
    ...
    for (const n of ['Jin Takamori', 'Haru Ishikawa', 'Hana Arakawa', 'Kai Moriyama']) expect(tierByName[n]).toBe('Established Paragon');
