...
import { parseRankBadge } from '../../shared/rankState';
...
export function LegendsPage() {
  ...
  return (
    <section>
      <PageHeader eyebrow="Recorded legends" title="Legends" description="The incidents that built the world’s reputations, rendered with the same dialogue colours, technique treatment, and rank-state pills as the novel reader." />
      ...
      <div className="lore-card-grid lore-card-grid--single">
        {visible.map((legend) => {
          const rank = parseRankBadge(legend.rank);
          return (
            <details className="lore-card" key={`${legend.rank}-${legend.title}`}>
              <summary><div className="tag-row">{rank ? <RankBadge rank={rank.rank} status={rank.status} /> : <span>{legend.rank}</span>}<span>{legend.kind}</span></div><h3>{legend.title}</h3><small>Read full legend ↓</small></summary>
