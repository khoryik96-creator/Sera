...
const entries = [
  ...
  // S109
  [109,444,'black_radiance','“Yes.”','Yurushi\'s expression did not change much, but the balance in his qi did. Light receded. Darkness thickened slightly around the edges of his circulation before settling again.'],
  ...
  [109,448,'rhen','“Yes.”','[aya] “You understand I healed people so they could return to the same war.”'],
  ...
  // S106 — only action-anchored cases; ambiguous alternation remains neutral.
...
for (const file of files.filter((f) => wanted.has(seasonOf(f)))) {
  ...
  let chapter = null;
  const targets = entries.filter(([s]) => s === season);
  for (let i = 0; i < paragraphs.length; i++) {
    ...
    for (const [s,ch,speaker,quote,prevAnchor,nextAnchor] of targets) {
      if (ch !== chapter || normalize(t) !== quote) continue;
...
console.log(`Added ${added} context-anchored speaker markers from ${entries.length} reviewed targets.`);
if (added !== entries.length) throw new Error(`Expected ${entries.length} context markers, added ${added}. Refusing partial silent success.`);
