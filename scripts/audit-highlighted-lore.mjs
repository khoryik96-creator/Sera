...
for (const match of registrySource.matchAll(entryPattern)) registry.push({ key: match[1], displayName: match[2], currentRank: match[6] || '' });
...
const results = registry.map((entry) => {
  ...
  return { key: entry.key, name: entry.displayName, sources: [profile && 'characters', arcFigure && 'arcFigures', former && 'former', castRows.length && `seasonCast(${castRows.map(r => `S${r.season}`).join(',')})`].filter(Boolean), strength, affiliation, role, summary, missing };
});

const incomplete = results.filter((row) => row.missing.length);
const noRichSource = results.filter((row) => !row.sources.length);
console.log(`TOTAL highlighted=${results.length} complete=${results.length - incomplete.length} incomplete=${incomplete.length} noRichSource=${noRichSource.length}`);
console.log('INCOMPLETE_BEGIN');
for (const row of incomplete) console.log(`${row.name}\tmissing=${row.missing.join(',')}\tstrength=${row.strength || '-'}\taffiliation=${row.affiliation || '-'}\trole=${row.role || '-'}\tsources=${row.sources.join('+') || '-'}`);
console.log('INCOMPLETE_END');
console.log('NO_RICH_SOURCE=' + noRichSource.map((row) => row.name).join(' | '));
