...
test('chapter archive exposes all 14 arcs and 69 seasons through direct selectors', async ({ page }) => {
  ...
  await expect(seasonSelect.locator('option')).toHaveCount(69);
  ...
  await expect(seasonSelect.locator('option').last()).toHaveAttribute('value', '69');
});
