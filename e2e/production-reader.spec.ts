...
test('chapter archive exposes every canonical arc and season through direct selectors', async ({ page }) => {
  ...
  await expect(arcSelect.locator('option')).toHaveCount(TOTAL_ARCS);
  await expect(seasonSelect.locator('option')).toHaveCount(TOTAL_SEASONS);
  ...
  await expect(seasonSelect.locator('option').last()).toHaveAttribute('value', String(TOTAL_SEASONS));
});
