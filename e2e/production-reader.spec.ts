...
test('chapter archive exposes all 16 arcs and 94 seasons through direct selectors', async ({ page }) => {
  ...
  await expect(arcSelect.locator('option')).toHaveCount(16);
  await expect(seasonSelect.locator('option')).toHaveCount(94);
  ...
  await expect(seasonSelect.locator('option').last()).toHaveAttribute('value', '94');
});
